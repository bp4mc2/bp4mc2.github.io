...
You might argue that the semantic world also stores information: into a table containing exactly three columns: subject, predicate, object. This is, of course, true. The difference is that the semantic world doesn't imply meaning by the way information is stored or written down in a message (with the exception that these three columns store assertions, as a triple containing a subject, predicate and object).

The value of a primary key of a table in a relation database uniquely identifies the *row* of data. In a triple the value of a subject only identifies the subject itself, and not the triple. The "primary key" of a triple is the combination of subject, predicate and object. The triples (or assertions) themselves don't have a single unique key.
...
Most URI's for properties and classes are so-called "#"-URI's, meaning that the document that describes these properties and classes automatically has a different URI from the properties and classes:

- [`<http://www.w3.org/2000/01/rdf-schema#label>`](http://www.w3.org/2000/01/rdf-schema#label) refers to a property (and technically it cannot result in a webpage response);
- [`<http://www.w3.org/2000/01/rdf-schema>`](http://www.w3.org/2000/01/rdf-schema) refers to a webpage, containing all triples that make up the rdf schema vocabulary.
...
At this moment, our resource is still "in the box": we haven't made an assertion whether it is a non-information resource, or an information resource. It seems a car, but it could be a document. We might include some versioning information:
	...
	<http://example.org/doc/car/WP0ZZZ99ZTS392124> a ex:Car;
		ex:color "red";
		ex:vehicleIdentificationNumber "WP0ZZZ99ZTS392124";
		ex:registrationDate "2015-12-05"^^xsd:date
	.
...
- `ex:Car` is actually "a document with data about a car". We might assert something like: `ex:Car` [`rdfs:subClassOf`](http://www.w3.org/2000/01/rdf-schema#subClassOf) [`foaf:Document`](http://xmlns.com/foaf/0.1/Document);
- `ex:color` is actually "the color of the car that is described by this document": the document isn't red, the car is red.
...
- `ex:registrationDate` is the data that a specific version of the document is registered (it is not the registration date of the car, because the date has changed after the change of the color, and the registration date of acar won't change due to a repainting of the car).
...
Because our vocabulary isn't about cars, but about cardata, we cannot reuse existing vocabularies about cars and it would be hard to link our cardata to other data. For example, if someone would like to assert that he owns our car, he might exidentally assert that he only owns some piece of information about the car...

The reason we cannot reuse vocabulaires and link to outside URI's about specific cars, is the common practice in the semantic world to directly refer to real-world objects, and to create vocabularies that have properties that directly refer to the subject, and not to some topic described by a document. While in relational databases it is the common practice to indirectly refer to real-life objects, the common practice in the semantic world is to directly refer to real-life objects. If we want to be part of the semantic world, we have to adhere to these common practices.
...
(You might argue that in a relational model these definitions could also be used for the "Vehicle" table and the "vehicleIdentificationNumber" column. However, in a relation model ALL references are indirect: nobody would say that the "Vehicle" table contains vehicles, it contains data about vehicles).
...
Let's see what happens if we would introduce non-information resources and reuse vocabularies:
	...
	<http://example.org/id/car/WP0ZZZ99ZTS392124> a schema:Car;
		...
		schema:color "red";
		foaf:isPrimaryTopicOf <http://example.org/doc/car/WP0ZZZ99ZTS392124/2015-12-05>
	.
	<http://example.org/doc/car/WP0ZZZ99ZTS392124/2015-12-05> a prov:Entity;
		prov:generatedAtTime "2015-12-05"^^xsd:date;
	.
	<http://example.org/id/car/WP0ZZZ99ZTS392124> schema:color "blue"
		foaf:isPrimaryTopicOf <http://example.org/doc/car/WP0ZZZ99ZTS392124/2017-05-15>
	.
	<http://example.org/doc/car/WP0ZZZ99ZTS392124/2017-05-15> a prov:Entity;
		prov:generatedAtTime "2017-05-15"^^xsd:date;
	.

By reusing existing vocabularies ([schema.org](http://schema.org), [foaf](http://xmlns.com/foaf/0.1/) and [prov](http://www.w3.org/ns/prov) in this case), users can immediately understand what the data means. And, because our car is now a real car, we can make assertions that the URI of our car refers to the same car as a URI from some other place, using for example [`owl:sameAs`](http://www.w3.org/2002/07/owl#sameAs). We still have documents with the description of that car, and assertions about the description, including links between the document and the real car (using [`foaf:isPrimaryTopicOf`](http://xmlns.com/foaf/0.1/isPrimaryTopicOf)).
